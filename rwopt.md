...
在memory db或者sstable的查找过程中，需要根据指定的序列号拼接一个internalKey，查找用户key一致，且seq号**不大于**指定seq的数据，具体的查找过程可见《Leveldb源码解析 - 内存数据库》和《Leveldb源码解析 - Sstable》两篇文章。
